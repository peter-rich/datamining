...
sss = StratifiedShuffleSplit(labels, train_size=0.54)
...
print(16)
model = Sequential()
model.add(Dense(hn,input_shape=(input_dim,),activation='tanh'))
model.add(Dropout(dp))
print(17)

for i in range(layers):
	model.add(Dense(hn, input_shape=(hn,),activation='tanh'))
	model.add(BatchNormalization())
	model.add(Dropout(dp))

model.add(Dense(output_dim,input_shape=(hn,)))
...
#model.fit(X_train, Y_train, nb_epoch=epochs, batch_size=batches,verbose=verbose,validation_data=(X_test,Y_test))
...
import gzip
with gzip.GzipFile('submission3.csv.gz',mode='w',compresslevel=9) as gzfile:
