...
###########################################################################################
#  the xgboost program is I found in the Internet wrong Firstly
#   but I fixed it successfully by myself which run 20 hours to 81.2 percent correct rates 
###########################################################################################
...
def main():
	...
	for z in train.cuisine.unique():
		v=encoder.inverse_transform(z)
		print("\nText Model: ",v,z)
		target = train.cuisine.apply(lambda x: 1 if x == z else 0)

		train['pred_text_'+v],text_models[v]=trainText(pipe_glm,grid_glm[v],pipe_xgb,grid_xgb[v],train.ingredients,target, cv,n_jobs=-1)
	...
	test[['id','cuisine']].to_csv("pred.csv",index=False)
